...
int CGame::GetStatus (void)
{
 ...
 /* Shows the splash string with playing instructions */
void CGame::ShowSplash (void)
{
 ...
 m_Console->OutputString ("are destroyed when then move to the \n");
...
void CGame::ShowGame (void)
{
 ...
 sprintf (Buffer, "Lives: %d   Score: %d   Leaps: %d   S - Save Game", 
          m_Player.GetLives () - 1, m_Player.GetScore (), 
          m_Player.GetLeaps ()); /* print*/
 Position.X = 0;
 Position.Y = 24;
 m_Console->SetPosition (Position);
...
void CGame::ShowLost (void)
{
 ...
 Position.Y = 12;
 m_Console->SetPosition (Position);
 m_Console->OutputString ("#             Though luck!             #");
 Position.Y = 13;
 m_Console->SetPosition (Position);
...
void CGame::ShowExit (void)
{
 ...
 m_Console->OutputString ("And remeber, stay away from drugs.\n\n");
}
...
{
  /* Since the spalh screen must be shown when we begin, we must
...
void CGame::Save (void)
{
 ...
 if (File.is_open ())
 {
  ...
  int   PlayerLeaps;
   ...
   /* Get information from player class */
  m_Player.GetPosition (&PlayerPosition);
