...
cMouse::cMouse(  HWND hWnd, bool bExclusive )
{
	...
	if( FAILED( hr ))
	{
		throw cGameError(L"[cMouse::Init]: Couldn't create the device!\n");
	}
 	...
 	if( FAILED( hr ))
	{
		...
		throw cGameError(L"[cMouse::Init]: SetDataFormat failed\n");
	}
	...
	{
		SafeRelease( m_pDevice );
		throw cGameError(L"[cMouse::Init]: SetCooperativeLevel failed\n");
	}
