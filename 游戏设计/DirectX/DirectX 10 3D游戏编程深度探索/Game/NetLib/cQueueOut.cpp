...
void cQueueOut::AddPacket( char *pData, unsigned short len )
{
  ...
  if( pPacket == NULL )
    throw cNetError( L"cQueueOut::AddPacket() - insufficient memory." );
...
// ReturnPacket() MUST be called or the mutex will not be released
// Also: do NOT delete the packet that you borrow
