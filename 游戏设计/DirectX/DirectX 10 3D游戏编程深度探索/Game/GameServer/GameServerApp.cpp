...
void AddText( TCHAR* text )
{
	static TCHAR buff[65536];

	SendMessage( g_ecWnd, WM_GETTEXT, 65536, (long)buff );

	wstring out = wstring(buff) + wstring(L"\r\n") + wstring(text);
...
LRESULT CALLBACK Dialog(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
	switch (message)
	{
		case WM_INITDIALOG:
			...
			SendMessage( g_ecWnd, WM_SETTEXT, 0, (long)L"Mobots ATTACK!\r\nServer Ver 10.5b\r\n==============" );
...
void AddPlayer( TCHAR* playerName )
{
	SendMessage( g_lbWnd, LB_ADDSTRING, 0, (long)playerName);
}

void RemovePlayer( TCHAR* playerName )
{
