...
cGameWorld::cGameWorld( cFile& file )
{
	...
	int nCells;
	swscanf_s( buff, L"NUM_CELLS %i", &nCells );
	...
	if( !IsClient() )
	{
		int nSpawnLocs;
		file.ReadNonCommentedLine( buff, '#' );
		swscanf_s( buff, L"NUM_SPAWN_LOCS %d", &nSpawnLocs );
		...
		for( i=0; i<nSpawnLocs; i++ )
		{
			...
			LogPrint(L"Creating Spawn point at %f %f %f",
...
void cGameWorld::RemovePlayer( objID id )
{
	...
	if( !pTarget )
	{
		LogPrint(L"cGameWorld::RemovePlayer: bad ID");
		return;
	}
	...
	{
		DP( L"ERROR: Tried to remove non-existent player\n" );
	}
	else
	{
...
UINT cGameWorld::ProcMsg( const sMsg& msg )
{
	...
	default:
		DP0(L"Bad Message got to cGameWorld\n");
		return -1;
	}
}
...
{
	LogPrint(L"[cGameWorld::NotifyRemove] Removing (%d:%d)", 
	...
	{
		LogPrint(L"[cGameWorld::NotifyRemove] object doesn't exist! can't remove." );
	}
...
objID cGameWorld::GetGoodSpawnEnt() 
{
	...
	DebPrint(L"found %d players", players.size() );
	...
	for( int i=0; i<nSpawnEnts; i++ )
	{
		...
		if( !pEnt )
		{
			LogPrint(L"cGameWorld::GetGoodSpawnEnt: bad ID");
			return -1;
		}
