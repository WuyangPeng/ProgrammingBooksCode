...
public:
	...
	static cNetMessage *ConstructMessage( unsigned char* pData, int dataSize);

protected:
	typedef std::map<unsigned long int, cNetMessageMaker *> tNetMessageMakerMap;
	static tNetMessageMakerMap m_registry;
	virtual cNetMessage *MakeMessage( unsigned char* pData, int datasize ) const = 0;
};
...
private: \
	cNetMessage* MakeMessage( unsigned char* pData, int datasize ) const \
	{ \
		cMsgName* pMsg = NULL; \
		try \
		{ \
