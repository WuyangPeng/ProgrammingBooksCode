...
cGameEnt::~cGameEnt()
{
	LogPrint(L"[cGameEnt::~cGameEnt] Entering destructor for (%d:%d)", 
...
bool cGameEnt::CheckState()
{
	...
	if( !pParent )
	{
		LogPrint(L"CheckState: bad parent ID");
		return true;
	}
	...
	if( bStillInParent )
	{
		...
		else
		{
			...
			if( portalNum == -1 )
			{
				 ...
				 * snap it back.
				 */
				//DebPrint(L"Ack! I have hit a wall!");
...
void cGameEnt::SetHealth( int newHealth )
{
	...
	DebPrint(L"Damage incurred.  new health is %d", newHealth );
...
UINT cGameEnt::ProcMsg( const sMsg& msg )
{
	...
	case msgNotifyPlaneCollision:
		DP0( L"performing default plane collision\n");
		AbutPlane( msg.m_plane );
		return 0; // the object is still alive and well.

	default:
		DP1( L"cGameEnt got unsup message [%x]", msg.m_type );
	}
	return 0;
}
...
void cGameEnt::AbutPlane( const plane3& plane )
{
	...
	{
		DP0(L"[ERROR!]: object got bogus AbutPlane!\n");
	}
}
...
bool MortisCheck( cGameEnt* pEnt )
{
	...
	{
		DebPrint(L"[MortisCheck]: Removing object %d%d", 
