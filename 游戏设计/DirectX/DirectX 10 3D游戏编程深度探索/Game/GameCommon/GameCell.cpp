	...
	m_index( num )
{
	...
	do
	{
		...
		else if( tok == L"PT" )
		{
			...
			// Read in the point
			tok = nextTok( tokQueue );
			pt.x = (float)_wtof( tok.c_str() );

			tok = nextTok( tokQueue );
			pt.y = (float)_wtof( tok.c_str() );

			tok = nextTok( tokQueue );
			pt.z = (float)_wtof( tok.c_str() );
		...
		else if( tok == L"FACE" )
		{
			...
			{
				tok = nextTok( tokQueue );
				currPoly.m_vList[i].m_ind = _wtoi( tok.c_str() );
			}
			...
			{
				tok = nextTok( tokQueue );
				swscanf_s( tok.c_str(), L"%x", &currPoly.m_vList[i].m_col );
			}
			...
			if( tok == L"AUTO" )
			{
				...
				tok = nextTok( tokQueue );
				currPoly.m_texID = _wtoi( tok.c_str() );

				tok = nextTok( tokQueue );
				float mScale = (float)_wtof( tok.c_str() );

				tok = nextTok( tokQueue );
				float nScale = (float)_wtof( tok.c_str() );
			...
			else if( tok == L"EXP" )
			{
				...
				// P
				tok = nextTok( tokQueue );
				p.x = (float)_wtof( tok.c_str() );

				tok = nextTok( tokQueue );
				p.y = (float)_wtof( tok.c_str() );

				tok = nextTok( tokQueue );
				p.z = (float)_wtof( tok.c_str() );

				// M
				tok = nextTok( tokQueue );
				m.x = (float)_wtof( tok.c_str() );

				tok = nextTok( tokQueue );
				m.y = (float)_wtof( tok.c_str() );

				tok = nextTok( tokQueue );
				m.z = (float)_wtof( tok.c_str() );

				// N
				tok = nextTok( tokQueue );
				n.x = (float)_wtof( tok.c_str() );

				tok = nextTok( tokQueue );
				n.y = (float)_wtof( tok.c_str() );

				tok = nextTok( tokQueue );
				n.z = (float)_wtof( tok.c_str() );

			}
			else throw cGameError( L"Bad texture gen token" );
		...
		else if( 0 == tok.compare( L"PORTAL" ) )
		{
			tok = nextTok( tokQueue );
			int other = _wtoi( tok.c_str() );
			...
			for( int i=0; i< nInd; i++ )
			{
				...
				currPortal.m_vList[i].m_ind = _wtoi( tok.c_str() );
			}
...
void cGameCell::AddObject( cGameEnt* pObj )
{
	...
	DP1(L"[cGameCell::AddObject]: Adding object %x\n", pObj );
}
...
bool cGameCell::ContainsObject( cGameEnt* pObj )
{
	...
	for( i=0; i<(int)m_polyList.size(); i++ )
	{
	...
	for( i=0; i<(int)m_portalList.size(); i++ )
	{
	...
	bool bFirst )
{
	...
	for( int i=0; i< (int)m_portalList.size(); i++ )
	{
		cGameCell* pOther = (cGameCell*)MsgDaemon()->Get( m_portalList[i].m_other );

		if( !pOther )
		{
			LogPrint(L"cGameCell::PortalWalk: bad cell");
			return;
		}
...
UINT cGameCell::ProcMsg( const sMsg& msg )
{
	...
	case msgRemObjRef:
		...
		break;
	default:
		DP0(L"Bad Message got to cGameCell\n");
		return -1;
	}
	return 0;
}
...
int cGameCell::sPortal::SnapInside( cGameEnt* pObj )
{
	...
	for( i=0; i< (int)m_edgePlanes.size(); i++ )
	{
