...
extern HANDLE hglobalmem;
extern LPSTR memAddress;
#define GLOBAL_BLOCK_SIZE 65536l
...
/* Memory functions */
boolean SRGP__memInit(void);
void    SRGP__memFree(void);
...
#ifdef _Windows
	typedef union {
		HANDLE		xid;	/* FOR ANY CANVAS */
		HWND		win;	/* ONLY FOR 0th CANVAS */
		HBITMAP		bitmap;	/* FOR ALL OTHER CANVASES */
	}
	drawable_type;
#endif
...
#ifdef _Windows
/*
DECLARE canvas_spec srgp__canvasTable[DEFAULT_MAX_CANVAS_INDEX+1];
*/
DECLARE HANDLE srgp__hInstance;
...
#ifdef _Windows
typedef HBRUSH				pattern_table_entry;
typedef HBITMAP				pixpat_table_entry;
#endif
...
#define SRGP_trace  if(!srgp__traceDisabled) SRGP_winTrace
#else
...
#define LeaveIfNonFatalErrReturn(VALUE)   \
   if(SRGP_errorOccurred)return VALUE
