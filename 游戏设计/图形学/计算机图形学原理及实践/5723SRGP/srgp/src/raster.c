...
/* This is NOT done correctly.  It only allows for bitblt on current screen */
   height = (source_rect.top_right.y - source_rect.bottom_left.y)+1;
   hdcSrc = GetDC (srgp__canvasTable[source_canvas_id].drawable.win);
   hdcDest = GetDC (srgp__curActiveCanvasSpec.drawable.win);
   BitBlt(hdcDest, dest_corner.x, DFIXED(dest_corner.y + height-1),
		  (source_rect.top_right.x - source_rect.bottom_left.x)+1,
		  height, hdcSrc, source_rect.bottom_left.x,
		  SFIXED(source_rect.bottom_left.y + height-1), SRCCOPY);
   ReleaseDC(srgp__canvasTable[source_canvas_id].drawable.win, hdcSrc);
   ReleaseDC(srgp__curActiveCanvasSpec.drawable.win, hdcDest);
}
#endif
}
