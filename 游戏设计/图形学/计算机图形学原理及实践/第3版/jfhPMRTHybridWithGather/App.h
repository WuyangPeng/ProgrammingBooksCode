/**
  @file App.h PHOTON MAP/RAY TRACE HYBRID with GATHER
...
#include "../jfhPhotonMapRayTraceHybrid/EPhoton.h"
#include "../jfhPhotonMapRayTraceHybrid/IPhoton.h"
#include "../jfhPhotonMapRayTraceHybrid/PhotonMap.h"
#include "../jfhPhotonMapRayTraceHybrid/LightList.h"
...
private:
	...
	bool                m_useGather;  // use final gathering? 
	...
	shared_ptr<GuiWindow>
		                m_resultsWindow; // A window in which current results are shown
    ...
    /*====================*/
    Radiance3 estimateTotalRadiance(const Ray& r, int depth, bool useGather);
    Radiance3 estimateEmittedLight(Ray r);
	Radiance3 estimateTotalScatteredRadiance(const Ray& r, int depth, bool useGather);

    Radiance3 estimateReflectedDirectLight(const Ray& r, const shared_ptr<Surfel>& surfel, int depth);
    Radiance3 estimateDiffuselyReflectedDirectLight(const Ray& r, const shared_ptr<Surfel>& surfel);
    Radiance3 estimateImpulseScatteredDirectLight(const Ray& r, const shared_ptr<Surfel>& surfel);

    Radiance3 estimateDiffuselyReflectedIndirectLight(const Ray& r, const shared_ptr<Surfel>& surfel, int depth, bool useGather);
    Radiance3 estimateImpulseScatteredIndirectLight(const Ray& r, const shared_ptr<Surfel>& surfel, int depth, bool useGather);
