...
void App::onInit() {
    ...
    toonShader = Shader::fromFiles("xtoon.vrt", "xtoon.pix");
	...
	xtoonMap = Texture::fromFile("xtoonmap.png", ImageFormat::RGB8(), Texture::DIM_2D);
...
void App::configureShaderArgs(const shared_ptr<Lighting> lighting, Args& args) {
	...
	args.setUniform("xtoonMap",            xtoonMap);
...
#   ifdef G3D_WINDOWS
      if (! FileSystem::exists("toon.pix", false) && FileSystem::exists("G3D.sln", false)) {
