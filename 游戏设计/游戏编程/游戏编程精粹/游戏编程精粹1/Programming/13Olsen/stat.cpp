...
{
    strncpy_s(_label, label, LABEL_MAX - 1);
...
int CStatEntry::Print([[maybe_unused]] char* outputBuffer, [[maybe_unused]] int maxLength)
{
    return 0;  // return #lines printed.
}

void CStatEntry::DeltaValue([[maybe_unused]] int delta)
{
    // We have no value to adjust.
}

// Set a stat with string data. Each type must convert the data to internal format.
void CStatEntry::SetValue([[maybe_unused]] char* name)
{
...
void CStatPage::Print(char* outputBuffer, int maxViewedLines)
{
    ...
    strcpy_s(outputBuffer, strlen(_label), _label);
    strcat_s(outputBuffer, 1, "\n");

    for (looper = _topEntry; looper != NULL && lineSum < maxViewedLines; looper = looper->_nextEntry)
    {
        strcpy_s(outputBuffer, strlen(looper->GetLabel()), looper->GetLabel());
        for (int i = static_cast<int>(strlen(looper->GetLabel())); i < LABEL_MAX; i++)
        {
            strcat_s(outputBuffer, 1, " ");
        }
        ...
        if (lineCurrent)
        {
            strcat_s(outputBuffer, strlen(printBuffer), printBuffer);
            strcat_s(outputBuffer, 1, "\n");
...
int CStatInt::Print(char* outputBuffer, [[maybe_unused]] int maxLength)
{
    sprintf_s(outputBuffer, 256, "%d", _value);
...
int CStatIntPtr::Print(char* outputBuffer, [[maybe_unused]] int maxLength)
{
    sprintf_s(outputBuffer, 256, "%d", *_value);
    return 1;  // We print one line of data into the buffer.
}
...
int CStatFloat::Print(char* outputBuffer, [[maybe_unused]] int maxLength)
{
    sprintf_s(outputBuffer, 256, "%.5f", _value);
    return 1;  // We print one line of data into the buffer.
}
...
int CStatFloatPtr::Print(char* outputBuffer, [[maybe_unused]] int maxLength)
{
    sprintf_s(outputBuffer, 256, "%.5f", *_value);
    return 1;  // We print one line of data into the buffer.
}
