...
void TextBox::Draw()
{
    Paginate();
    Begin();
    if (mode & TEXT_DRAW_BOX)
        DrawBoundingBox();
    DrawStr(static_cast<u16>(boxL), static_cast<u16>(boxT), 1, static_cast<u16>(boxB), drawBufferPtr);
    End();
}
...
void TextBox::DrawStr(u16 x, u16 y, u16 maxFlag, u16 maxY, char* string)
{
    ...
    while (*cP != '\0')
    {
        ...
        else if (' ' <= *cP && *cP <= '~')
        {
            ...
            glVertex3s(static_cast<GLshort>(cursorX), static_cast<GLshort>(cursorY), 0);
            glTexCoord2f((float)(s + FONT_W) / FONT_TEX_W, (float)(t) / FONT_TEX_H);
            glVertex3s(static_cast<GLshort>(cursorX + FONT_DRAW_W), static_cast<GLshort>(cursorY), 0);
            glTexCoord2f((float)(s + FONT_W) / FONT_TEX_W, (float)(t + FONT_H + 1) / FONT_TEX_H);
            glVertex3s(static_cast<GLshort>(cursorX + FONT_DRAW_W), static_cast<GLshort>(cursorY + FONT_DRAW_H), 0);
            glTexCoord2f((float)(s) / FONT_TEX_W, (float)(t + FONT_H + 1) / FONT_TEX_H);
            glVertex3s(static_cast<GLshort>(cursorX), static_cast<GLshort>(cursorY + FONT_DRAW_H), 0);
            cursorX += FONT_DRAW_W;
        }
        if (*cP == '\n')
        {
...
void TextBox::DrawBoundingBox()
{
    ...
    glBegin(GL_QUADS);
    glVertex3s(static_cast<GLshort>(boxL), static_cast<GLshort>(boxT), 0);
    glVertex3s(static_cast<GLshort>(boxR), static_cast<GLshort>(boxT), 0);
    glVertex3s(static_cast<GLshort>(boxR), static_cast<GLshort>(boxB), 0);
    glVertex3s(static_cast<GLshort>(boxL), static_cast<GLshort>(boxB), 0);
    glEnd();
}
...
void TextBox::FormatStrCat(char* str)
{
    ...
    while (*cP != '\0')
    {
        ...
        if (currX >= boxR)
        {
            ...
            *cP = '\0';
            // Eliminate Space
            if (mode & TEXT_WRAP_SPACES && buff2[0] == ' ')
                cP2 = &buff2[1];
            else
                cP2 = buff2;
            strcat_s(cP, MAX_STR, cP2);
            currX = boxL;
        }
        else
        {
    ...
    {
        strcpy_s(buff2, cP);
        strcpy_s(buffer, 4096, buff2);
    }
    // Now there is space. So, concatinate.
    strcat_s(buffer, 4096, buff);
}
