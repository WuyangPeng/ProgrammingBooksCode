...
GameSession::GameSession()
{
    ...
    m_influenceMap = NULL;
    m_influenceMap = new OccupanceInfluenceMap();
    m_influenceMap->Init(32,32,INITIAL_WORLD_SIZE,INITIAL_WORLD_SIZE);
    m_influenceMap->DrawGrid();
    m_influenceMap->DrawInfluence();
}
...
void GameSession::PostGameObj(GameObj *obj)
{
    ...
    if(m_influenceMap && obj->m_influence)
        m_influenceMap->RegisterGameObj(obj);
}
...
bool RemoveNotActive(GameObj* obj)
{
    if(!obj->m_active)
    {
        if(obj->m_influence && Game.GetInfluenceMap())
            Game.GetInfluenceMap()->RemoveGameObj(obj);
