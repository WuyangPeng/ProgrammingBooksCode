...
Group::Group( char * label, CWnd * pWin, const int level ){
	...
	m_numChildWidgets = 0;
	m_widgetBank = 0;
	for (int i = 0; i < MAX_CHILD_GROUPS; i++ )
	{
		m_childGroups[i] = 0;
	}
...
*
*****************************************************************************/
Group* Group::AddGroupWidget( char * label )
{
	Group * w;
					 ...
					 int ID1, EventHandler * h )
{
	...
	r.MoveToY(GetClientHeight() + MINIMIZED_WIDGET_HEIGHT);
...
int Group::Draw( int y_pos )
{
	...
	if (IsExpanded()){
		for ( unsigned int i = 0, y_pos = MINIMIZED_WIDGET_HEIGHT; i < m_numChildGroups; i++ )
		{
			m_childGroups[i]->GetWindowRect(r3);
			if ( m_prevPos != abs(r3.top - r.top ) )
			{
				y_pos += ((Group *)m_childGroups[i])->Draw( y_pos );
			}
			m_prevPos = abs( r3.top - r.top );
		}
		for (unsigned int i = 0; i < m_numChildWidgets; i++)
		{
			m_childWidgets[i]->Draw();
		}
	}
	return height;
}
