...
*				can house child widgets, or child groups (which can 
*				in turn host other widgets, other groups, and so on). 
*				It contains a header which can be expanded/contracted 
*				to show/hide its child groups or child widgets, which 
*				are added with subsequent calls to AddScrubber, 
*				AddOnOff, etc.
*
********************************************************************/
...
public:
	...
	RadioButton * AddRadio( char * groupName, char * leftName, char * rightName, 
		int & val, int id1, int id2, EventHandler * h = 0 );
...
private:
	...
	int m_prevPos;					// used for positioning groups when drawing
