...
inline void lpcd_propertysetup(lua_State *L, int metaTableIndex)
{
	lua_pushstring(L, "__index");		// m s
	lua_pushcclosure(L, LPCD::PropertyMetaTable_index, 0);	// m s c
	lua_rawset(L, -3);					// m

	lua_pushstring(L, "__newindex");	// m s
	lua_pushcclosure(L, LPCD::PropertyMetaTable_newindex, 0);	// m s c
	lua_rawset(L, -3);					// m
}


template <typename Object, typename VarType>
inline void lpcd_propertyregister(lua_State *L, int metaTableIndex, const char* varName, VarType Object::* var)
{
	lua_pushstring(L, "__props");					// s
	lua_rawget(L, metaTableIndex);					// props
	if (lua_isnil(L, -1))
	{
		lua_pop(L, 1);								//
		lua_newtable(L);							// props
		lua_pushstring(L, "__props");				// props s
		lua_pushvalue(L, -2);						// props s props
		lua_rawset(L, metaTableIndex);				// props
	}

	lua_newtable(L);								// props prop
	lua_pushstring(L, varName);						// props prop varName
	lua_pushvalue(L, -2);							// props prop varName prop
	lua_rawset(L, -4);								// props prop

	lua_pushnumber(L, 1);							// props prop 1
	lpcd_pushmemberpropertygetclosure(L, var);		// props prop 1 get
	lua_rawset(L, -3);								// props prop

	lua_pushnumber(L, 2);							// props prop 2
	lpcd_pushmemberpropertysetclosure(L, var);		// props prop 2 get
	lua_rawset(L, -3);								// props prop
