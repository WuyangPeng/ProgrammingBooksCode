...
//---------------------------------------------------------
GameObj* GameSession::GetGameObj(int pid)
{
    //go through the list, find the object with m_ID that matches
    //param "pid"
    GameObjectList::iterator list1;
    for(list1=m_activeObj.begin();list1!=m_activeObj.end();++list1)
    {
        if((*list1)->m_messReceiver->m_ID == pid)
            return *list1;
    }
    return NULL;
