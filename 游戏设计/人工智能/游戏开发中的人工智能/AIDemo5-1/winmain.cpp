...
Example:        Potential function based movement, Chapter 5
*/
//---------------------------------------------------------------------------
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	switch (message) {
		...
		case WM_ACTIVATE:
			...
			state = Swarm ? MF_CHECKED:MF_UNCHECKED;
			CheckMenuItem(GetMenu(hMainWindow), ID_OPTIONS_SWARM, state);
			
			state = Chase ? MF_CHECKED:MF_UNCHECKED;
			CheckMenuItem(GetMenu(hMainWindow), ID_OPTIONS_CHASEMOUSE, state);

			state = Avoid ? MF_CHECKED:MF_UNCHECKED;
			CheckMenuItem(GetMenu(hMainWindow), ID_OPTIONS_AVOID, state);
