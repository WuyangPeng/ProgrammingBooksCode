...
void NetworkManager::TryAdvanceTurn()
{
    ...
    if (static_cast<int>(mTurnData[mTurnNumber + 1].size()) == mPlayerCount)
    {
        if (mState == NMS_Delay)
        {
...
void NetworkManager::ProcessPacketsHello(InputMemoryBitStream& inInputStream, [[maybe_unused]] const SocketAddress& inFromAddress)
{
