...
void NetworkManager::TryAdvanceTurn()
{
    ...
    if (mTurnData[mTurnNumber + 1].size() == static_cast<size_t>(mPlayerCount))
    {
        if (mState == NMS_Delay)
        {
...
void NetworkManager::HandleReadyPacket([[maybe_unused]] InputMemoryBitStream& inInputStream, [[maybe_unused]] uint64_t inFromPlayer)
{
