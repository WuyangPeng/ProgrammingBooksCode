...
ScoreBoardManager::Entry::Entry(uint64_t inPlayerId, const string& inPlayerName, const Vector3& inColor)
...
void ScoreBoardManager::AddEntry(uint64_t inPlayerId, const string& inPlayerName)
{
    //if this player id exists already, remove it first- it would be crazy to have two of the same id
    RemoveEntry(inPlayerId);

    mEntries.emplace_back(inPlayerId, inPlayerName, mDefaultColors[mEntries.size() % mDefaultColors.size()]);
}
