...
CCurve::CCurve(CPoint* pFirst, CPoint* pSecond, COLORREF aColor)
{
   // Store the points
   m_Points.push_back(pFirst);
   m_Points.push_back(pSecond);
   m_Color = aColor;
   m_EnclosingRect = CRect(min((*pFirst).x, (*pSecond).x), min((*pFirst).y, (*pSecond).y),
                           max((*pFirst).x, (*pSecond).x), max((*pFirst).y, (*pSecond).y));
   m_PenWidth = 1;
}
...
{
   m_Points.push_back(pPoint);                // Add the point to the end

   // Modify the enclosing rectangle for the new point
   m_EnclosingRect = CRect(min((*pPoint).x, m_EnclosingRect.left),
                           min((*pPoint).y, m_EnclosingRect.top),
                           max((*pPoint).x, m_EnclosingRect.right),
                           max((*pPoint).y, m_EnclosingRect.bottom));
}
