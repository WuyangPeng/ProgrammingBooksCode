// Soln6_03CLR.cpp : main project file.
// A CLR calculator supporting parentheses and identifying input  errors.

// The error() function marks the character position where an error is detected.
// This requires the current position in the input string to be determined so the error
// character can be flagged, even in recursive function calls.
// The error() function is called in the number() function and in the expr() function.

// In the standard C++ calculator, you can use global variables to track the index position
// in the string for error tracking purposes.
// You cannot create C++/CLI variables at global scope so to keep track of the index position in the
// input string, so the original string and the inputIndex variable that is created in main()
// and passed to the expr() function are used for this.

// When a parenthesised subexpression is recognized in the number() function, the inputIndex
