	...
	{ for(size_t WARMUPS = 2; 0 != WARMUPS; --WARMUPS)
	{
		...
		{ for(size_t r = 0; r < nRepeats; ++r)
		{
...

			{
				...
				for(token = ::strtok_r(s, SEP, &last_s); NULL != token; token = ::strtok_r(NULL, SEP, &last_s))
				{
...

			{
				...
				for(n = 0, token = ::strtok_r(s, SEP, &last_s); NULL != token; token = ::strtok_r(NULL, SEP, &last_s))
				{
					++n;
				}
			}

			{
				...
				for(l = 0, token = ::strtok_r(s, SEP, &last_s); NULL != token; token = ::strtok_r(NULL, SEP, &last_s))
				{
					l += ::strlen(token);
				}
			}
