...
///  单例的生存期。
///  SmallObjects的特殊生存期：
///  当所持有的对象是Small(Value)Object或所持有的对象使用从Small(Value)Object继承或继承自Small(Value)Object对象时，您将无法使用默认生存期。
///  必须使用生存期代码 Loki::LongevityLifetime::DieAsSmallObjectChild。
///  当您使用Loki::Factory，Loki::Functor或Loki::Function时请注意这一点。
