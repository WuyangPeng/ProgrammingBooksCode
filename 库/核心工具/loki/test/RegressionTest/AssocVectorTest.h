...
private:
    void operator=(const TestAllocator&) = delete;
};
...
{
public:
    AVTestClass(int value)
        : value_(value)
    {
    }
    AVTestClass(const AVTestClass& other)
        : value_(other.value_)
    {
    }
...
void test_vect2()
{
    ...
    assert(!aux.second);
    assert(aux.first->first == std::string{ "xyz" });
...
void test_vect3()
{
    ...
    {
        char buffer[17];
        C_Namespace::sprintf_s(buffer, 16, "string%d", C_Namespace::rand());
        std::string s(buffer);
...
void test_vect6()
{
    ...
    for (unsigned int i0 = 0; i0 < numTests; ++i0)
    {
        IntMap testVec;
        for (unsigned int i1 = 0; i1 < numSamples; ++i1)
        {
