...
void OutputResults( unsigned int loop, size_t value, size_t result )
{
...
void * ValueSafeThread( void * p )
{
    ...
    const size_t value = reinterpret_cast<size_t>(p);
    volatile Thing & thing = Thing::GetIt();
    try
    {
        for ( unsigned int ii = 0; ii < testCount; ++ii )
        {
            ...
            ::GoToSleep( 3 );
            const auto result = thing.GetValue();
...
void * TryLockThread( void * p )
{
    ...
    for ( unsigned int ii = 0; ii < 5; ++ii )
    {
        MutexErrors result = mutex.TryLock();
        ...
        const auto gotValue = thing.GetValue();
...
void MultiThreadTryLockTest( void )
{
    ...
    pool.JoinAll();
    const auto value = thing.GetValue();
...
void SingleThreadComplexMultiLockTest( bool doSetup )
{
    ...
    MutexErrors result = LevelMutexInfo::MultiLock( mutexes );
...
void * MultiLockSafeThread( void * p )
{
    ...
    try
    {
        for ( unsigned int ii = 0; ii < thingCount; ++ii )
        {
            ...
            [[maybe_unused]] bool okay = locker.Unlock();
            assert( okay );
            ::GoToSleep( 2 );
        }
    }
