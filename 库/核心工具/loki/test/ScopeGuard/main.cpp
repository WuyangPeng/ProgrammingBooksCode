...
void User::AddFriendGuarded(User& newFriend)
{
    ...
    friends_.push_back(&newFriend);
    [[maybe_unused]] Loki::ScopeGuard guard = Loki::MakeObjGuard(friends_, &UserCont::pop_back);
    
    fCount++;
    [[maybe_unused]] Loki::ScopeGuard guardRef = Loki::MakeGuard(Decrement, Loki::ByRef(fCount));
